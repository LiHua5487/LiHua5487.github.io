...
本笔记不包含python特别基础的东西，如常见变量类型，基本语句等（这种东西随便一搜都有，实在想看见下面的参考链接）
部分笔记参考了[某位同学的笔记](https://lh314-pku.github.io/notes/Programming_Internship/index)（放这里只是怕以后找不到他的主页了）
但是，这个笔记加入了很多口语性的描述与简单易懂的例子（而不是按照传奇程设ppt），也是本人初见理解的过程（怕以后忘了）
...
print(a)
```
输出的是 `[4,5,6,7]` ，因为 `a = b` 后二者指向的地方一样，改 b 也会 改 a
...
对于列表，`+=` 是直接对列表操作，`l = l + [num]` 是新建个列表再赋给 `l`
这就导致了如下情况
```python
a = [1,2,3]
b = a
...
- 遍历冲突的哈希桶，使用 `__eq__` 判断相等性
    ...
    - `a.__eq__(A(3))` 返回 `True`，因此找到对应的值 `20`
...
这个例子中，我们希望每次执行add和multiply都能打印日志信息，于是将其提取出来放到装饰器
